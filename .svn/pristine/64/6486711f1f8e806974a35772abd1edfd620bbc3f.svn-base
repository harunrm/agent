...
namespace MISL.Ababil.Agent.UI
{
    public class GUI
    {
                ...
                switch (ControlsForValidation.ElementAt(i).Value)
                {
                    ...
                    case ValidCheck.VALIDATIONTYPES.TEXTBOX_NUMBER:
                        //ValidCheck.IsNumber(((MetroTextBox)ControlsForValidation.ElementAt(i).Key), null);
                        {
                            ...
                            if (ValidCheck.IsNumber(ref control, null))
                            {
                                result = false;
                            }
                            else
                    ...
                    case ValidCheck.VALIDATIONTYPES.TEXTBOX_DATE:
                        //code here.
                        break;
                    case ValidCheck.VALIDATIONTYPES.TEXTBOX_NationalID:
                        //ValidCheck.IsNumber(((MetroTextBox)ControlsForValidation.ElementAt(i).Key), null);
                        {
                            MetroTextBox control = (MetroTextBox)ControlsForValidation.ElementAt(i).Key;
                            Control parent = control.Parent;

                            if (ValidCheck.IsNationalID(control, null))
                            {
                                result = true;
                            }
                            else
        ...
        public void Config(ref TextBox control, ValidCheck.VALIDATIONTYPES validationType, string message)
        {
            ControlsForValidation.Add(control, validationType);
            GUI_Events gui_events = new GUI_Events();
            control.Enter += gui_events.control_Enter;
        ...
        public class GUI_Events
        {
            ...
            public void control_KeyUp_TextBoxIsEmptyValidation(object sender, KeyEventArgs e)
            {
                if (e.KeyCode == Keys.Return)
                {
                    ...
                    {
                        if (ValidCheck.IsEmpty((MetroTextBox)control, null))
                        {
                            //IsValidFocus = true;
                        }
                        else
                    ...
                    {
                        if (ValidCheck.IsEmpty((TextBox)control, null))
                        {
                            //IsValidFocus = true;
                        }
                        else
            ...
            public void control_KeyUp_TextBoxIsNationalIDNullableValidation(object sender, KeyEventArgs e)
            {
                if (e.KeyCode == Keys.Return)
                {
                    ...
                    {
                        Message.showError("Invalid National ID");
                        g.DrawRectangle(Pens.Red, textBox.Left - 1, textBox.Top - 1, textBox.Width + 1, textBox.Height + 1);
                        return;
                    }
                    else if (!ValidCheck.IsNationalID((MetroTextBox)sender, ""))
                    {
                        Message.showError("Invalid National ID");
                        g.DrawRectangle(Pens.Red, textBox.Left - 1, textBox.Top - 1, textBox.Width + 1, textBox.Height + 1);
                        return;
        ...
        public static class DeveloperMode
        {
            ...
            public static void EnableDeveloperMode(Form frm)
            {
                ...
                Message.showInformation("DEVELOPER MODE IS ACTIVATED!");
            }
