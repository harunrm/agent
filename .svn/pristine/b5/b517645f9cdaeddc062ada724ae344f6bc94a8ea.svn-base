...
namespace MISL.Ababil.Agent.UI.forms
{
    public partial class frmDocument : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (gvDocument.Rows.Count > 0)
            {
                ...
                try
                {
                    ...
                    if (retrnMsg != "")
                    {
                        Message.showInformation("The document is saved.");
                    }
                    this.Close();
                }
                catch (Exception ex)
            ...
            {
                //btnSave.DialogResult = DialogResult.None;
                Message.showError("There is no document to save");
            }
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (validationCheck())
            {
                try
                {
                    ...
                    {
                        Message.showError("Please enter the Expire Date in correct format.");
                        //mtb2.Focus();
                        //mtb2.SelectAll();
                        return;
                    }
                    ...
                    if (txtCode.Text == "")
                    {
                        Message.showError("Input Code not found.");
                        return;
                    }
                    if (bytes == null)
                    {
                        txtFilePath.Text = "";
                        Message.showError("Take a photo or Browse a file.");
                        return;
                    }
                    if (IsEdit == true)
                    {
        ...
        private void gvDocument_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (docInfo.documents != null)
            {
                ...
                if (senderGrid.Columns[e.ColumnIndex] is DataGridViewButtonColumn && e.RowIndex >= 0)
                {
                    ...
                    if (e.ColumnIndex == 0)
                    {
                        string result = Message.showConfirmation("Are you sure to delete document?");
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            string result = Message.showConfirmation("Are you sure to clear?");
